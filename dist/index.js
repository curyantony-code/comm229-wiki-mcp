...
const WIKI_ROOT = (() => {
    ...
    if (!root) {
        process.stderr.write("Error: provide wiki path as first argument or WIKI_ROOT env var\n" +
...
const server = new Server({ name: "comm229-wiki", version: "1.1.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        switch (name) {
            ...
            case "wiki_read": {
                const content = await readPage(args.path);
                return { content: [{ type: "text", text: content }] };
            }
            case "wiki_list": {
                const files = await listCategory(args.category);
            ...
            case "wiki_write": {
                await writePage(args.path, args.content);
                return { content: [{ type: "text", text: `Written: ${args.path}` }] };
            }
            case "wiki_log": {
                await appendLog(args.entry);
